...
export const DEFAULT_CONFIG = {
  ...
  getGroup: node => {
    if (node.dataset.simplebox && node.dataset.simplebox !== "true") {
      return {attr: "data-simplebox", group: node.dataset.simplebox}
    } else if (node.dataset.fancybox && node.dataset.fancybox !== "true") {
...
class SBModal {
  ...
  handleClick = ev => {
    // bg click handling has some nuance: several elements are treated as "background". The height
    // cutoff is so that near-miss taps below the thumbnails don't accidentally close.
    const
      innerBody = this.rootEl.querySelector(".simplebox-modal__inner-body"),
      treatAsBgEls = [
        this.rootEl,
